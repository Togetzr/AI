...
class WhatsappResourseFlow(PortalReports):
    ...
    def get_resource_calendar_attendance_slot(self, data=None):
        ...
        combination_id = request.env['resource.booking.combination.type'].sudo().search([('id','=',int(data.get("data").get("resourse",False)))]) if data.get('data').get('resourse',False) and data.get('data').get('resourse',False) != 'AutoAssign' else False
        ...
        resource_ids = type_id.resource_ids
        ...
        if data.get("data").get("appointment",False):
            ...
            for booking_time in booking_times:
                ...
                end_time = datetime.combine(day, float_to_time(booking_time.hour_to)) - timedelta(hours=resource_booking_type.duration)
                # _logger.info(f"start_time-start_time-start_time {start_time}")
                current_time = start_time
                day_records = request.env['resource.booking'].sudo().search([('start', '>=', day), ('stop', '<', day + timedelta(days=1)), ('combination_id', 'in', sorted_combinations.ids),('type_id','=',resource_booking_type.id)])
                
                while current_time <= end_time:
                    next_time = current_time + timedelta(hours=resource_booking_type.slot_duration)
                    start_current = current_time - timedelta(hours=resource_booking_type.duration)
                    stop_current = current_time + timedelta(hours=resource_booking_type.duration)
                    
#                    _logger.info("---start_current   %s"%start_current)
#                    _logger.info("---stop_current   %s"%stop_current)
                    
                    paris_timezone = pytz.timezone(company_time_zone)
                    paris_start_current = paris_timezone.localize(start_current)
                    paris_stop_current = paris_timezone.localize(stop_current)
                    
#                    _logger.info("---paris_start_current   %s"%paris_start_current)
#                    _logger.info("---paris_stop_current   %s"%paris_stop_current)
                    
                    start_current = paris_start_current.astimezone(pytz.utc)
                    stop_current = paris_stop_current.astimezone(pytz.utc)
                    
#                    _logger.info("---start_current   %s"%start_current)
#                    _logger.info("---stop_current   %s"%stop_current)
                    particular_bookings = request.env['resource.booking'].sudo().search([('start', '>', start_current), ('start', '<', stop_current), ('id', 'in', day_records.ids)])
                    _logger.info("---particular_bookings   %s"%particular_bookings)
#                    utc_timezone = pytz.utc
#                    utc_time = utc_timezone.localize(record.start)
                    
                    slot_available = True if len(sorted_combinations) > len(particular_bookings) else False
                    
                    slots.append({
                        "id":f"{current_time.strftime('%H:%M')}",
                        "title":f"{current_time.strftime('%H:%M')}",
...
#                    current_time = next_time
                ...
                if data.get("screen") == "QUESTION_ONE":
                    if data.get("data").get("component_action",False) and data.get("data").get("component_action",False) == 'update_resource':
                        if data.get("data").get("resourse",False):
                            booking_combination = request.env['resource.booking.combination.type'].sudo().search([('id','=',int(data.get("data").get("resourse",False)))])
                            combination_rel = request.env['resource.booking.type.combination.rel'].sudo().search([('combination_id','in',booking_combination.combination_ids.ids)])
    ...
    #                            "resourse_source":resourse_source,
                ...
                if data.get("data").get("component_action",False) == 'update_booking_type':
                    ...
                    resourse_source = [
                        {"id":str(x.id),
                        "title":x.name} for x in request.env['resource.booking.combination.type'].sudo().search([])
                    ]
                    services_source = [{
                                        "id":str(x.id),
                                        "title":x.name
