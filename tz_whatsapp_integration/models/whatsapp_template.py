...
class WhatsAppTemplate(models.Model):
    ...
    wa_account_id = fields.Many2one('res.company', string="Account",default=lambda self:self.env.user.company_id)
    ...
    cron_job = fields.Char(string="Cron Job")
    has_action = fields.Boolean(string="Has Action", compute='_compute_has_action')
    message_ids = fields.One2many('mail.message','whatsapp_template_id')
    default_template = fields.Boolean(string="Default Message Template")
    contact_exist = fields.Boolean(string="Send if Contact exist",default=True,help="Used when we are sending message for first time")
    send_location = fields.Boolean(string="Send Location Button")
    lat_field = fields.Char(string="Latitude Field")
    lng_field = fields.Char(string="Longitude Field")
    reply_when_receive_location = fields.Char(string="Put method after receive reply of location")
    mess_type = fields.Selection([('template','Whatsapp Template'),('interactive','Interactive')],default='template',string="Message Type",required=True)
    trigger_word = fields.Char(string="Trigger Word")
    is_mpm_template = fields.Boolean(string="MPM Message",copy=False) 
...
#                raise ValidationError(_('Free text variable in the header should be {{1}}'))
    ...
    def _compute_variable_ids(self):
        ...
        for tmpl in self:
            ...
            update_commands = [(3,to_delete_id) for to_delete_id in to_delete] + [(0,0,vals) for vals in to_create]
            if update_commands:
    ...
    def button_submit_template(self):
        ...
        if attachment:
            # Open session
            file_handle = self.env['whatsapp'].upload_demo_document(attachment,self.wa_account_id)
        ...
        if self.wa_template_uid:
            self.env['whatsapp'].whatsapp_api_requests("POST", f"/{self.wa_template_uid}",company_id=self.wa_account_id, data=json_data)
            self.status = 'pending'
        else:
            response = self.env['whatsapp'].whatsapp_api_requests("POST", f"/{self.wa_account_id.waba_id}/message_templates", data=json_data,company_id=self.wa_account_id)
            self.write({
    ...
    def button_sync_template(self):
        ...
        self.ensure_one()
        response = self.env['whatsapp'].whatsapp_api_requests("GET", f"/{self.wa_template_uid}",company_id=self.wa_account_id)
        if response.get('id'):
    ...
    def _update_template_from_response(self, remote_template_vals):
        ...
        variable_ids.extend([(3,0) for to_remove in existing_template_variables.values()])
        update_vals['variable_ids'] = variable_ids

        for button in template_vals['button_ids']:
            button['variable_ids'] = [(0,0,var) for var in button['variable_ids']]
        self.button_ids = [(6,0,[])]
        update_vals['button_ids'] = [(0,0,button) for button in template_vals['button_ids']]
        ...
        if not self.header_attachment_ids or self.header_type != template_vals['header_type']:
            ...
            update_vals['header_attachment_ids'] = [(6,0,[])] + new_attachment_commands
    ...
    def _get_header_component(self, free_text_json, template_variables_value, attachment,company_id):
        ...
        if header_type == 'text' and template_variables_value.get('header-{{1}}'):
            ...
            header = {
                'type': 'header',
                'parameters': [self._prepare_attachment_vals(attachment, company_id=company_id)]
            }
    ...
    @api.model
    def _prepare_attachment_vals(self, attachment, company_id):
        """ Upload the attachment to WhatsApp and return prepared values to attach to the message. """
        whatsapp_media_type = attachment.mimetype

        if not whatsapp_media_type:
            raise WhatsAppError(_("Attachment mimetype is not supported by WhatsApp: %s.", attachment.mimetype))
        
        whatsapp_media_uid = self.env['whatsapp'].upload_document(attachment,company_id=self.wa_account_id)
        
        vals = {
            'type': 'document',
            'document': {
                'id': whatsapp_media_uid,
    ...
    def _get_interactive_button(self):
        component = []
        ...
        for button in self.button_ids:
            if button.button_type == 'url':
                ...
                component.append({
                    "type":"reply",
                    "reply": {
                        "id": button.name.lower().replace(" ","_"),
                        "title": button.name
                      }
                })
    ...
    def _get_send_template_vals(self, record, free_text_json, attachment=False,company_id=False):
        ...
        header = self._get_header_component(free_text_json=free_text_json, attachment=attachment, template_variables_value=template_variables_value,company_id=company_id)
    ...
    def button_create_action(self):
        """ Create action for sending WhatsApp template message in model defined in template. It will be used in bulk sending"""
        view = self.env.ref('tz_whatsapp_integration.whatsapp_composer_view_form')
        for tmpl in self:
            action = self.env['ir.actions.act_window'].sudo().search([('res_model', '=', 'whatsapp.composer'), ('binding_model_id', '=', tmpl.model_id.id)])
            if not action:
