...
class WhatsAppTemplateButton(models.Model):
    ...
    button_type = fields.Selection([
        ...
        ('flow','Send Flow')], string="Type", required=True, default='quick_reply')
    ...
    replied_with_template = fields.Many2one('whatsapp.template',string="Reply with template")
    replied_with_message = fields.Char(string="Reply with Message")
    call_method_after_click = fields.Char(string="Call method after the execute button")
    replied_with_flow = fields.Many2one('whatsapp.flow',string="Reply with flow")
    ...
    def open_configuration(self):
        return {
            'name': _("Button Configuration"),
            'view_mode': 'form',
            'res_model': 'whatsapp.template.button',
            'res_id':self.id,
